...
//uint8 protocal_test[SIZE_MAX_SEND_BUFFER] = { 0, };
...
union
 ...
 int adc_value1; int adc_value2;
void send_input_data(int GHI,int adc_valued)
{
  ...
  adc_16_val.aaddcc_value = adc_valued;
  protocal_test[5] = crc_16_val.b[0];
  protocal_test[6] = crc_16_val.b[1];
  protocal_test[7] = adc_value1=adc_16_val.bbbbdata[0];
  protocal_test[8] = adc_value2=adc_16_val.bbbbdata[1];
  protocal_test[9] = '\n';

  //Ifx_SizeT g_count = sizeof(crc_16_val.b);
  //IfxAsclin_Asc_write(&g_ascHandle, crc_16_val.b, &g_count, TIME_INFINITE);
}
...
void asclin0RxISR(void)
{
        ...
        for(int i =0; i<MAX_RCV_BUFFUR_SIZE-1; i++)
        {
            g_rxRcvBuffer[i] = g_rxRcvBuffer[i+1];
        }
        g_rxRcvBuffer[MAX_RCV_BUFFUR_SIZE-1] = g_rxData[0];
        cnt_rcv++;*/
}
...
void send_ASCLIN_UART_message(int GHI,int adc_valued)
{
    double_count++;
    send_input_data(GHI,adc_valued);
...
void BYTE_SIZE(int btye)
{
    if(btye >= 128) { a[7] = 1; btye -= 128;}
        else { a[7] = 0;}
    if(btye >= 64) { a[6] = 1; btye -= 64;}
        else { a[6] = 0; }
    if(btye >= 32) { a[5] = 1; btye -= 32;}
        else { a[5] = 0;}
    if(btye >= 16) { a[4] = 1; btye -= 16;}
        else { a[4] = 0;}
    if(btye >= 8) { a[3] = 1; btye -= 8;}
        else { a[3] = 0; }
    if(btye >= 4) { a[2] = 1; btye -= 4;}
        else { a[2] = 0;}
    if(btye >= 2) { a[1] = 1; btye -= 2;}
        else { a[1] = 0;}
    if(btye >= 1) { a[0] = 1; btye -= 1;}
        else { a[0] = 0;}
