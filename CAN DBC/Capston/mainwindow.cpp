...
void MainWindow::on_Open_ReadFiles_clicked()
{
    ...
    if (fileDBC.open(QIODevice::ReadOnly | QIODevice::Text))
    { // 파일 열기 시도
...
void MainWindow::on_NewConnection()
{
    ...
    headerLabels << "Throttling"<<"RPM"<<"KM/h"<<"ENG_Temp"<<"Steer_Angle";

    ui->CAN_Data_Table->setHorizontalHeaderLabels(headerLabels);
...
void MainWindow::decode_Value(const QString& _data)
{
    QStringList parts = _data.split(' ',Qt::SkipEmptyParts);
    int ID= parts[0].right(2).toInt(nullptr,16);
    if(ID != ui->Address->text().toInt(nullptr))
    {
        return;
    }
    ...
    int DI_chan= parts[2].toInt(nullptr,16);
    bool DI_Condi[8]={0,};
    for(int i=0;i<8;i++)
    {
        DI_Condi[i]=DI_chan>>i &0x01;
    }
    DI_Condi[0] == true?ui->DI1->setStyleSheet("background-color: #FFFF00;"):ui->DI1->setStyleSheet("background-color: #FFFFFF;");
    DI_Condi[1] == true?ui->DI2->setStyleSheet("background-color: #FFFF00;"):ui->DI2->setStyleSheet("background-color: #FFFFFF;");
    DI_Condi[2] == true?ui->DI3->setStyleSheet("background-color: #FFFF00;"):ui->DI3->setStyleSheet("background-color: #FFFFFF;");
    DI_Condi[3] == true?ui->DI4->setStyleSheet("background-color: #FFFF00;"):ui->DI4->setStyleSheet("background-color: #FFFFFF;");
    DI_Condi[4] == true?ui->DI5->setStyleSheet("background-color: #FFFF00;"):ui->DI5->setStyleSheet("background-color: #FFFFFF;");
    DI_Condi[5] == true?ui->DI6->setStyleSheet("background-color: #FFFF00;"):ui->DI6->setStyleSheet("background-color: #FFFFFF;");
    DI_Condi[6] == true?ui->DI7->setStyleSheet("background-color: #FFFF00;"):ui->DI7->setStyleSheet("background-color: #FFFFFF;");
    DI_Condi[7] == true?ui->DI8->setStyleSheet("background-color: #FFFF00;"):ui->DI8->setStyleSheet("background-color: #FFFFFF;");

}

void MainWindow::on_DI_clicked(bool checked)
{
    DI_Check=checked;
}

int lived=0;
void MainWindow::on_DO_clicked(bool checked)
{
    DO_Check=checked;
}

void MainWindow::CAN_DI_Part(){
    if(DI_Check && lived)
    {
...
void MainWindow::CAN_DO_Part()
{
    if(DO_Check && lived)
    {
        bool DO_Condi[8]={0,};
        int DO_data=0;
        DO_Condi[0]= ui->DO1->currentIndex();
        DO_Condi[1]= ui->DO2->currentIndex();
        DO_Condi[2]= ui->DO3->currentIndex();
        DO_Condi[3]= ui->DO4->currentIndex();
        DO_Condi[4]= ui->DO5->currentIndex();
        DO_Condi[5]= ui->DO6->currentIndex();
        DO_Condi[6]= ui->DO7->currentIndex();
        DO_Condi[7]= ui->DO8->currentIndex();
        for(int i=0;i<8;i++)
        {
            DO_data+=DO_Condi[i]<<i;
        }
